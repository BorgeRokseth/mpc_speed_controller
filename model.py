...
class ShipModelSimplifiedPropulsion:
    ...
    def __init__(self, ship_config: ShipConfiguration,
        ...
        route = simulation_config.route
        self.navigate = NavigationSystem(route)
        self.next_wpt = 1
        self.prev_wpt = 0
    ...
    def rudderang_from_route(self):
        ...
        self.next_wpt, self.prev_wpt = self.navigate.next_wpt(self.next_wpt, self.n, self.e)
        psi_d = self.navigate.los_guidance(self.next_wpt, self.n, self.e)
        if self.next_wpt == self.prev_wpt: # we have reached the end of the route
            self.game_over = True
        return self.rudderang_from_headingref(psi_d)
...
class NavigationSystem:
    ...
    def __init__(self, route: List[WayPoint]):
        self.list_of_waypoints = route
        self.load_waypoints()
        self.ra = 300  # Radius of acceptance for waypoints
    ...
    def load_waypoints(self):
        ...
        self.north = []
        self.east = []
        for waypoint in self.list_of_waypoints:
            self.north.append(waypoint.north)
            self.east.append(waypoint.east)
